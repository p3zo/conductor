 ...
 */
export function ComputePreprocInfoFromBoxCoords(coords, ar, slack) {
    if (coords.length !== 6)
        throw 'wrong num coords';
    ...
    const rotationCenter = MathUtil.DivideVectors(MathUtil.AddVectors(coords[0], coords[1]), [2.0, 2.0]);
    const araRot = new MathUtil.AspectRatioAwareRotation(rotationCenter, rotationInRadians, ar);
...
function AddBoxSlack(boxCoords, topLeftBottomRight, ar, slack) {
 ...
 */
export function ComputePreprocInfoFromLanCoords(coords, ar, slack) {
    const palmBottom = coords[20];
    const palmTop = coords[5];
    const rotationCenter = MathUtil.DivideVectors(MathUtil.AddVectors(palmBottom, palmTop), [2.0, 2.0]);
...
function AddLanSlack(topLeftBottomRight, ar, slack, coords) {
    const distances = [];
...
function ComputeTopLeftBottomRightFromExtremumCoords(extremumCoords) {
...
export class SixPointRotationCalculation {
    constructor(palmBottom, palmTop, aspectRatio) {
...
//# sourceMappingURL=preproc_comp.js.map
import { MirrorCoordinatesHorizontally, ApplyPaddingToCoordinates, ConvertLandmarkCoordinatesToGlobalCoordinates, MakeCoordinateOrderUserFriendly, AssembleTrackResultFromCoordinatesAndClasses, } from '../post_model/post_model';
import { ComputePreprocInfoFromBoxCoords, ComputePreprocInfoFromLanCoords, } from '../pre_model/preproc_comp';
 ...
 * analysis.
 */
export async function StartEngine(config, trackSource, preprocCb, boxCb, lanCb, resCb) {
    ...
    while (!stopped) {
        if (!preprocInfo) {
            ...
            preprocInfo = ComputePreprocInfoFromBoxCoords(boxRes.coordinates, aspectRatio, config.__boxSlack);
        }
        await RequestAnimationFrame();
        ...
        let coords = ConvertLandmarkCoordinatesToGlobalCoordinates(preprocInfo, aspectRatio, lanRes.coordinates);
        preprocInfo =
        ...
        const result = AssembleTrackResultFromCoordinatesAndClasses(coords, classes);
        resCb(result);
 ...
 */
export function ApplyPostProcessingToCoordinates(config, coords) {
    if (config.padding) {
